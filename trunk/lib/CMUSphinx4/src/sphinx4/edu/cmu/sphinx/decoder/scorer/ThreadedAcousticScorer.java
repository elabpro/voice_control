...
public class ThreadedAcousticScorer extends SimpleAcousticScorer {
    ...
    /**
     * The property that controls the service priority of scoring threads.
     ...
     * to have one service per CPU available to score states, set the NUM_THREADS property to 0 and the isCpuRelative to
     * true. If you want exactly one service to process scores set NUM_THREADS to 1 and isCpuRelative to false.
     * <p/>
     * If the value is 1 isCpuRelative is false no additional service will be instantiated, and all computation will be
     * done in the calling service itself. The default value is 0.
     */
     ...
     * zero. Also, if the number of threads is 0, the states are scored on the calling service, no separate threads are
    ...
    /**
     * The property that controls the minimum number of scoreables sent to a service. This is used to prevent
    ...
    private int minScoreablesPerThread; // min scoreables sent to a service
     ...
     *            want to have one service per CPU available to score states, set
     *            the NUM_THREADS property to 0 and the isCpuRelative to true.
     *            If you want exactly one service to process scores set
     *            NUM_THREADS to 1 and isCpuRelative to false.
     *            <p/>
     *            If the value is 1 isCpuRelative is false no additional service
     *            will be instantiated, and all computation will be done in the
     *            calling service itself. The default value is 0.
     * @param cpuRelative
     ...
     *            the states are scored on the calling service, no separate
     *            threads are started. The default value is false.
     * @param numThreads
     *            the minimum number of scoreables sent to a service. This is
     ...
     *            the service priority of scoring threads. Must be a value between
    ...
    protected <T extends Scoreable> T doScoring(List<T> scoreableList, final Data data) throws Exception {
        ...
        // if no additional threads are necessary, do the scoring in the calling service
